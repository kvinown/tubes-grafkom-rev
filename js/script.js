...
class DonutRain {
	constructor() {
		...
		this.donutTexture = this.textureLoader.load("assets/donut/textures/donut_normal.png");
	}
	...
	createDonuts() {
		for (let i = 0; i < this.numDonuts; i++) {
			...
			donut.position.set(Math.random() * 40 - 5, Math.random() * 10 + 10, Math.random() * 5 - 10);
...
const loadModel = () => {
	...
	const material = new THREE.MeshStandardMaterial({
		...
		emissive: 0x000000,
		flatShading: true,
		metalness: 0, // Menambahkan sifat metalik
		roughness: 0.4, // Menambahkan sifat kekasaran permukaan
	});
...
const getRandomColor = () => {
	// Menghasilkan warna acak dengan nilai RGB
...
const addToppings = (donut) => {
	...
	for (let i = 0; i < 2500; i++) {
		...
		// Membuat material baru dengan warna acak untuk setiap meses
		...
		// Mengatur orientasi agar meses menghadap ke luar donut
		meses.lookAt(new THREE.Vector3(x - radius * Math.cos(theta), y - radius * Math.sin(theta), z));
